...
@WebServlet("/square/fetch_particular")
public class FetchParticular extends HttpServlet {
        ...
        try{
            if(TestInfo.testSocialgroupId(socialgroup_id) && TestInfo.testUser_id(user_id) && TestInfo.testPassword(password)
                ...
                if(square_item_type.equals("broadcast")){
                    fetchBroadcast(req, resp, user_id, socialgroup_id, square_item_id);
                }else if(square_item_type.equals("circle")){
                    fetchCircle(req, resp, user_id, socialgroup_id, square_item_id);
                }
     ...
     * @param resp
     */
    private void fetchBroadcast(HttpServletRequest req, HttpServletResponse resp,String user_id,  String socialgroup_id, String square_item_id) throws SQLException, ClassNotFoundException {
        ...
        String sql_like = "SELECT square_item_id FROM judge WHERE square_item_id = ? " +
                "AND from_user_id = ? AND square_item_type = 1 AND judge_type = 1";

        String sql_dislike = "SELECT square_item_id FROM judge WHERE square_item_id = ? " +
     ...
     * @param resp
     */
    private void fetchCircle(HttpServletRequest req, HttpServletResponse resp, String user_id, String socialgroup_id, String square_item_id) throws SQLException {
